...
阻塞：在不考虑跳转语句的前提下，计算机的指令是逐条进行的，上一条执行完事之后才会执行下一条，当某条指令是要求进行I/O操作的时候，计算机会调用更底层的东西进行I/O操作，在这期间，计算机就会等待底层的命令返回结果，然后再执行下一个指令。这种进程等待底层返回结果再执行下一条指令的现象就是阻塞(I/O阻塞了进程)

非阻塞的意思就是，主进程不需要等待I/O返回结果，继续向下执行，等I/O操作有了返回结果，通知主进程就可以。

CPU密集：纯粹的CPU运算，比如压缩，解压，加密，解密操作

I/O密集：文件操作，网络操作，数据库

NodeJs的单线程只针对主进程，I/O操作由系统底层进行多线程调度。
