...
> 简单值总是通过复制来进行赋值和传递，复合值(对象或者函数)总是通过引用复制的方式来赋值/传递

### 判断数据类型：

1. `typeof`：返回数据类型的字符串表达。`typeof`对变量执行操作的时候，得到的结果并不是该变量的类型，而是给变量持有值的类型，因为变量是没有类型的。

   > undefined表示从未赋值，null 表示曾经被赋值，但是现在是空值　
...
+ `+`运算符会将显式地将操作数转化为数字，例如+"3.14"，其实是将3.14的字符串转化为数字，比较常见的语法就是 `var timestamp = +new Date`(需要注意的是，如果构造函数没有参数，可以不带构造函数后面的花括号)
+ 非字符串在与字符串转换的时候，+是直接调用valueOf函数，而String()是调用toString()函数。
...
相同函数名，不同参数列表的多个函数，在调用时可以根据传入参数的不同，自动选择对应的函数执行。函数存在两个隐式的参数，一个是`this`，还有一个是`arguments`。`this`表示被调用函数的上下文对象。`arguments`可以让我们访问函数调用过程中传递的实际参数。
...
对于函数调用，也就是说被调用的函数表达式不是作为一个对象的属性存在的，这个时候函数上下文就有两种可能性，非严格模式下，它将是全局上下文window对象，而在非严格模式下，它将是undefined。
