...
this没有作用域的限制，this并不受函数作用域的限制，一个函数A的内部定义一个函数B，在B的内部并不能继承A函数的this，如果嵌套函数作为方法调用，其this的值指向调用它的对象，嵌套函数作为函数调用，其this值不是全局对象就是undefined。如果想要访问这个外部函数的this值，需要将this的值保存在一个变量里。<b>用一句话概括，谁调用的这个函数，这个函数的this就指向谁。</b>
