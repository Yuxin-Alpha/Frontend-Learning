...
第一个字符表示文件类型，d表示目录，-表示文件，|表示链接文件，c表示外设

后面9个，每三个表示一组权限，一共三组，第一组表示文件所有者的权限，第二组表示文件所属用户组的权限，第三组表示其他人的权限，r表示可读，w表示可写，x表示可执行，如果没有权限则会被-代替
...
+ `make -p /home/animal/tiger`创建多级目录
