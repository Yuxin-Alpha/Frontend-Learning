...
> 这里的条件控制没有小括号包裹，需要注意，而且条件语句中可以进行赋值操作
...
`Golang`只是支持面向对象编程特性，没有方法重载，没有构造函数，没有析构函数，没有`this`，而是通过结构体实现, 而结构体是一个值类型
